...
svclassifier = SVC(kernel='linear',coef0=3)
...
for k in range(20):
    ...
    for i in range(len(selected_features)):
        x_train_selected_features[str(selected_features[k][i])] = x_train.iloc[:,int(selected_features[k][i])] # X_Train from selected features
        x_test_selected_features[str(selected_features[k][i])] = x_test.iloc[:,int(selected_features[k][i])] # # X_Test from selected features
        reduced_dataset['attr ' + str(int(selected_features[k][i]))] = dataset.iloc[:,int(selected_features[k][i])]
        
    reduced_dataset['class'] = dataset.iloc[:,-1]  
    reduced_dataset.to_csv('D:/Project/Thermal FER/subset' + str(k) + '.csv', index=False) 
